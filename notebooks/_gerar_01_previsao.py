"""Gera notebooks/01_previsao_saldo.ipynb — sem nbformat, JSON direto."""
import json
...
def _linhas(texto: str) -> list[str]:
    """
    Cada string de `source` precisa TERMINAR em \\n, menos a última.

    O formato .ipynb guarda a célula como lista de linhas JÁ com o separador.
    Dividir por "\\n" e guardar sem ele faz o Jupyter concatenar tudo numa linha
    só — foi exatamente o que aconteceu: `import duckdbimport numpy as np`.
    O JSON fica válido e o notebook ABRE; o erro só aparece ao executar.
    """
    linhas = texto.strip("\n").split("\n")
...
# Confere ANTES de gravar: célula que não compila não vai para o repositório.
# É a checagem que faltou na primeira versão — JSON válido não garante Python
# válido, e o erro só apareceu no Colab de quem abriu.
import re as _re
...
for i, c in enumerate(celulas):
    if c["cell_type"] != "code":
        continue
    fonte = "".join(c["source"])
    fonte = _re.sub(r"^%.*$", "pass", fonte, flags=_re.M)   # magics do Jupyter
    try:
        compile(fonte, f"<celula {i}>", "exec")
    except SyntaxError as e:
        raise SystemExit(f"celula {i} nao compila (linha {e.lineno}): {e.msg}\n"
                         f"---\n{fonte[:300]}")
print(f"{sum(1 for c in celulas if c['cell_type'] == 'code')} celula(s) de codigo compilam")

DESTINO.parent.mkdir(parents=True, exist_ok=True)
DESTINO.write_text(json.dumps(nb, ensure_ascii=False, indent=1), encoding="utf-8")
print(f"{DESTINO}  ({len(celulas)} celulas, {DESTINO.stat().st_size / 1024:.0f} KB)")
